...
export const emailRegister = action(RegisterSchema, async ({ email, password, name}) => {
    ...
    if (existingUser) {
        if(!existingUser.emailVerified) {
            // const verificationToken = // we will make a token that will be sent to the email and it needs to ne saved to our database with the user that made the request.
        }
        return {error: "Email already in use"}
    }
    return {success: "Woooww congratulations"}
